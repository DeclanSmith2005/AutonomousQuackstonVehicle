...
def ignore_intersection(px, speed):
    ...
    record_motion_sample(motion_history, speed, config.STRAIGHT_ANGLE, eyes.last_line_seen)
...
def recover_by_retracing(px, eyes, motion_history, pid):
    """
    Reverse through recent motion history and invert steering
    to approximately retrace the recent path until the line is seen again.
    """
    ...
    # Find the most recent sample where the line was still visible
    last_good_idx = None
    ...
    px.stop()
    pid.reset()
    time.sleep(config.RECOVERY_SETTLE_TIME)
...
def main():
    ...
    def _handle_roundabout_exit_prep(raw):
        ...
        if bool(config.ROUNDABOUT_EXIT_USE_BRANCH_TRIGGER):
            ...
            record_motion_sample(motion_history, current_motor_speed, config.STRAIGHT_ANGLE, eyes.last_line_seen)
    ...
    try:
        while not stop_flag:
            ...
            if not eyes.last_line_seen:
                ...
                # Recovery attempt by retracing history if enabled and cooldown has elapsed
                lost_duration = time.time() - last_valid_line_time

                recovery_allowed = (
                    config.RECOVERY_ENABLED and 
                    (time.time() - last_recovery_time) >= config.RECOVERY_COOLDOWN_SEC
                )
                ...
                if lost_duration > config.RECOVERY_TRIGGER_TIMEOUT and recovery_allowed:
                    ...
                    recovered = recover_by_retracing(px, eyes, motion_history, pid)
