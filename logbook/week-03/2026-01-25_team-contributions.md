---
title: "Team Contributions - Summary of Individual Contributions to the Project Proposal Report"
date: 2026-01-25
week: 3
hours: X.X
tags: Report
contributors: Ishaan Grewal, Rafael Costa, Nolan Su-Hackett, Declan Smith
---

## Log Summary

The purpose of this log is to simply track/summarize each individual's contributions to the Project Proposal Report. Under each individual's entry, they have reported the
total number of hours they spent on the Project Proposal Report, and listed the sections of the report they contributed to (whether that is editing or writing).

---

## Entry – Ishaan Grewal

**Activity Type:** Report Writing/Editing 
**Estimated Time Spent:** 14.0 h  

### Work Performed
**Sections Written/Work Completed:**
- (2. Proposed Technical Approach) - 2.1 Functional Requirements & Constraints & 2.2 High-Level Overview of Proposed System: Main Layers & Their Performance
- 3.2.1 Perception - 3.2.1.1.1 Training Data & 3.2.1.2 Key Requirements & 3.2.1.3.1 Object Detection Pipeline & 3.2.1.3.2 Sense Pipeline
- 3.2.2 Localization
- (4. Project Management Plan) - 4.1 Team Roles and Responsibilities & 4.2 Project Management & Scheduling
- Created flowcharts for the Object Detection Pipeline and Sense Pipeline
- Set up the team's Project Management Kanban and Gantt Chart in Github

**Other Work/Sections Edited:**
- Contributed to editing all sections of the report for grammar, spelling, in-text referencing/citations, and flow/cohesion.
- Contributed ideas to other sections as well.

### Reflection

Overall, I believe the team was able to create an excellent report which met all rubric requirements with immense detail, clarity, and cohesion. The report was
structured, and the use of flowcharts to explain the implementation of each subsystem made the proposed technical approach very clear and easy to understand. Going
forward, I believe it is valuable to have team review sessions (like we did for this report), since it helps identify areas of improvement and encourages meaningful
discussions, which improve the quality of the report. Since the team was struggling with converting the report to .PDF format when submitting and had to email the
professor about this technical issue, it is best that the team does not make significant formatting edits (like updating table of contents or figures) when a lot
of people are editing the report, as we believe this was the reason that the file could not be converted (proposed changes were potentially backlogged).

---

## Entry – Nolan Su-Hackett

**Activity Type:** Report Writing/Editing 
**Estimated Time Spent:** 10.0 h   

### Work Performed
**Sections Written/Work Completed:**
- High Level system overview Flow chart
- 3.2.1 Perception
- 5. Decision Making and Conflict Resolution
- 7. Risk, Safety and Ethical Considerations


**Other Work/Sections Edited:**
- Helped in reading and editing all sections


### Reflection
Organization of the report was well thought out, multiple meetings took place within each sub group and overall as each team ran into issues (mostly with interfacing) to ensure that everything came together. The intention of the report was not just to get something down on paper but to get something down that was functional
and would have real application for the project. The team was intentional about making realistic decisions especially when it came to interfacing and this report acted as a 
large planning and framework step that will allow the team to move forward with a plan and well organized approach.

---

## Entry – [Team Member Name 3]
 
**Activity Type:** [Implementation / Testing / Design / Documentation / Project Management / Hardware]   
**Estimated Time Spent:** X.X h  

### Work Performed
**Sections Written/Work Completed:**
- 


**Other Work/Sections Edited:**
- 


### Reflection


---

## Entry – [Team Member Name 4]
 
**Activity Type:** [Implementation / Testing / Design / Documentation / Project Management / Hardware]   
**Estimated Time Spent:** X.X h  

### Work Performed
**Sections Written/Work Completed:**
- 


**Other Work/Sections Edited:**
- 


### Reflection


---

---

**Entry completed**: YYYY-MM-DD HH:MM
